...
class WC_Product_Gift_Wrap
{
	 ...
	 *
	 * @since 1.0.0
	 * @var string $message_also_for_no_gift_wrap Message to be displayed.
	 */
		...
		if (empty($cart_item['gift_wrap'])) {
			if ($this->message_also_for_no_gift_wrap != 'no') {
				$item_data[] = array(
					...
					'value'   => __('No', 'product-gift-wrap-for-woocommerce'),
					'display' => __('No', 'product-gift-wrap-for-woocommerce'),
				);
			}
	...
	public function admin_settings()
	{
		...
		$this->settings = array(
			...
			array(
				...
				'type' 		=> 'checkbox',
				'desc_tip'  => __('Explicite message shown to the user and to shop  manager if NO gift wrap required.', 'product-gift-wrap-for-woocommerce'),
			),
		);
